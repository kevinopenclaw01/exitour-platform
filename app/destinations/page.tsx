...
import { countries, destinations } from "@/lib/data";
    ...
    <>
      ...
      <section className="bg-slate-50">
        <div className="mx-auto max-w-7xl px-5 py-10 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-black tracking-tight text-slate-950">국가별로 보기</h2>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
            {countries.map((country) => (
              <Link
                key={country.slug}
                href={`/destinations/${country.slug}`}
                className="rounded-lg border border-slate-200 bg-white px-4 py-4 text-sm font-black text-slate-900 shadow-sm transition hover:-translate-y-0.5 hover:border-cyan-300 hover:text-cyan-700 hover:shadow-md"
              >
                {country.name}
              </Link>
            ))}
